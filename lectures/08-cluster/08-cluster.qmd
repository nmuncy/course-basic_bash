...
::::: columns
:::: {.column width="50%" style="font-size: 65%;"}
- From a web browser go to [https://crcfe01.crc.nd.edu](https://crcfe01.crc.nd.edu)
...
## Moving data: scp
:::: {style="font-size: 65%;"}
- `scp`: Copy files over an SSH connection
    - Copies entire file or directory
- Anatomy of command:
    - `scp` [OPTS] source destination
    - Separate computer from path with '`:`'
- Can be used to both push and pull data:
    - `scp file.txt user@crcfe01.crc.nd.edu:~`
        - Send `file.txt` from local to CRC `home`
    - `scp -r user@crcfe01.crc.nd.edu:~/project ~/Desktop`
        - Pull `~/project` directory from CRC to the local desktop
- See [here](https://linuxize.com/post/how-to-use-scp-command-to-securely-transfer-files/) for more details
:::::

## Moving data: rsync
:::: {style="font-size: 65%;"}
- Do not often desire to send entire directory
    - e.g. files exist at `src` and `dst`, new files made at `dst`
- `rsync`: Index both `src` and `dst`
    - Move only difference
- Anatomy of command:
    - `rsync` [OPTS] source destination
- Similar to `scp`, can both push and pull data
    - `rsync -rauv ~/project user@crcfe01.crc.nd.edu:~`
    - Compares local `project` to remote, sends any difference
- See [here](https://linuxize.com/post/how-to-use-rsync-for-local-and-remote-data-transfer-and-synchronization/) for more details
:::::

## Moving data: Graphic User Interface
:::: {style="font-size: 65%;"}
- GUI-based options available
    - [Cyberduck](https://cyberduck.io/download/) (OSx)
    - [MobaXterm](https://mobaxterm.mobatek.net/) (Windows)
- The CRC has documentation [here](https://docs.crc.nd.edu/new_user/quick_start.html#file-transfers)
:::::